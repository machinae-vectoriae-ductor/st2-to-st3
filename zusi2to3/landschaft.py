...
def conv_ls(filename, no_displacement=False):
    ...
    with open(outname_abs, "w") as fout:
        ...
        if len(verknuepfungen):
            ...
            for verkn in verknuepfungen:
                fout.write(
                    f'<Verknuepfte SichtbarBis="3000" BoundingR="{verkn.boundingr}">\n<Datei Dateiname="{verkn.dateiname_zusi}"/>\n<p X="{verkn.x-centerx}" Y="{verkn.y-centery}" Z="{verkn.z}"/>\n<phi X="{verkn.rx}" Y="{verkn.ry}" Z="{verkn.rz}"/>\n</Verknuepfte>\n'
                )
        else:
            boundingr = 0
